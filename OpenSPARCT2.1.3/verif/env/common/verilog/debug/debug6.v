...
// 
// OpenSPARC T2 Processor File: debug6.v
...
`endif
  ...
  if (enabled) `PR_ALWAYS ("dbg_chk", `ALWAYS, "Core 6 Debug Logic Checker debug6.v is alive and enabled.");
  else `PR_ALWAYS ("dbg_chk", `ALWAYS, "!!!Core 6 Debug Logic Checker NOT enabled!!!");
  if ($test$plusargs("noCmtCheck")) noCmtCheck = 1;
  if (noCmtCheck) `PR_ALWAYS ("dbg_chk", `ALWAYS, "!!!Core 6 Debug Logic Commit Checker NOT enabled!!!");
...
always @(negedge `SPC6.tlu.tcu_core_running[0]) begin: GOTPARK0
...
always @(posedge `SPC6.tlu.tcu_core_running[0]) begin: UNPARKCHECK0
...
// check for unexpected running transition
  always @(`SPC6.tlu.tlu_core_running_status[0]) begin: UNEXPECTED0
...
begin
  ...
  repeat (2) @(negedge `SPC6.l2clk);

  // need to see at least one assertion of flush
  // if (`SPC6.tlu_flush_ifu[0] !== 1) 
  //   @(posedge `SPC6.tlu_flush_ifu[0]); // has glitches
  if (`SPC6.ifu_ftu.ftu_agc_ctl.tlu_flush_ifu_f[0] !== 1)
    @(posedge `SPC6.ifu_ftu.ftu_agc_ctl.tlu_flush_ifu_f[0]);

  // need to see all of these go idle
  while (`SPC6.pku_quiesce[0] == 0 ||
         `SPC6.ftu_ifu_quiesce[0] == 0 ||
         `SPC6.lsu_stb_empty[0] == 0) 
     @(negedge `SPC6.l2clk);
...
begin //{
  ...
  while ((parkTrans[0] == 0 && ssMode == 0) || 
         (virtualPark[0] == 1 && ssMode == 1 && 
          `TOP.cpu.tcu_ss_request[6] == 0)) begin //{
    `PR_DEBUG ("dbg_chk", `DEBUG, "TID 0 watching for activity while parked/between steps. trans=%0h, virtualPark=%0h, tcu_ss_request=%0h",parkTrans[0],virtualPark[0],`TOP.cpu.tcu_ss_request[6]);
    ...
    while (`SPC6.tlu.tcu_core_running[0] == 0 && 
           `SPC6.tlu.tlu_core_running_status[0] == 1) 
      @(negedge `SPC6.l2clk);

    // watch for instructions sneaking in
    // dec_inst_valid_m[1:0], one bit per group
    if (`SPC6.tlu_trap_pc_0_valid && `SPC6.tlu_trap_0_tid[1:0] == 0 && `SPC6.tlu.tlu_retry[0] == 1)
      `PR_ERROR ("dbg_chk", `ERROR, "ERROR: TID 0 had tlu_trap_pc_0_valid activity while parked/between steps!");

    if (`SPC6.dec_inst_valid_m[0] == 1 && `SPC6.dec_tid0_m[1:0] == 0)
      `PR_ERROR ("dbg_chk", `ERROR, "ERROR: TID 0 had dec_inst_valid_m activity while parked/between steps!");


  end //} while

  `PR_DEBUG ("dbg_chk", `DEBUG, "TID 0 STOP watching for activity while parked/between steps. trans=%0h, virtualPark=%0h, tcu_ss_request=%0h",parkTrans[0],virtualPark[0],`TOP.cpu.tcu_ss_request[6]);
...
begin
  ...
  // must see `SPC6.tlu.core_running_status[0] w/in count clocks
  count0 = 0;

  while (`SPC6.tlu.tlu_core_running_status[0] == 0) begin
    @(negedge `SPC6.l2clk);
  ...
  if (!doMode && !ssMode) begin
    ...
    // happened at once so while loop can be skipped.
      while (! (`SPC6.tlu_trap_pc_0_valid && 
                `SPC6.tlu_trap_0_tid[1:0] == 0)) begin
        @(negedge `SPC6.l2clk);
...
begin
  ...
  fork
    // timeout fork
    begin : FORK10_0
      repeat (stepWaitTime) @(negedge `SPC6.l2clk);
        `PR_ERROR ("dbg_chk", `ERROR, "watchStep0: TID 0 did not see valid PC retry (waitTime=%0d)!", stepWaitTime);
    end

//     // look for SPU going active    
//     begin : FORK11_0
//       while (!(`SPC6.spu_tlu_cwq_busy || (`SPC6.spu.mct.spu_pmu_ma_busy[3] && `SPC6.spu.mct.spu_pmu_ma_busy[2:0] == 0)))
//         @(negedge `SPC6.l2clk);
...
//     end
    ...
    begin : FORK9_0
      ...
      while (!(`SPC6.tlu_trap_pc_0_valid && `SPC6.tlu_trap_0_tid[1:0] == 0 && `SPC6.tlu.tlu_retry[0] == 1)) begin
        // might see  `SPC6.tlu.mmu_reload_done[0] here if previous
        ...
        if (`SPC6.tlu.mmu_reload_done[0] && ssMode) begin
      ...
      @(negedge `SPC6.pku_quiesce[0]);    
      `PR_DEBUG("dbg_chk", `DEBUG, "watchStep0: TID 0 got negedge pku_quiesce, step waiting on dec_inst_valid_m!");
    
      while (!(`SPC6.dec_inst_valid_m[0] == 1 && `SPC6.dec_tid0_m[1:0] == 0)) @(negedge `SPC6.l2clk);
    
      `PR_DEBUG("dbg_chk", `DEBUG, "watchStep0: TID 0 valid step starting!");
      disable FORK10_0; // kill timeout
      //disable FORK11_0;
      //       if (!`SPC6.spu_tlu_cwq_busy && 
//           !(`SPC6.spu.mct.spu_pmu_ma_busy[3] && 
//             `SPC6.spu.mct.spu_pmu_ma_busy[2:0] == 0)) spuActive[0] = 0;
    end
  join
  ...
  fork
    ...
    // look for SPU going active    
//     begin : FORK12_0
//       while (!(`SPC6.spu_tlu_cwq_busy || 
//         ((`SPC6.spu.mct.spu_pmu_ma_busy[3] && `SPC6.spu.mct.spu_pmu_ma_busy[2:0] == 0 && doMode) || (`SPC6.spu.mct.spu_pmu_ma_busy[3] && ssMode))))
//         @(negedge `SPC6.l2clk);
//       spuActive[0] = 1;
//     end
    ...
    begin : FORK6_0
        ...
        while (`SPC6.tlu_flush_ifu[0] == 0)
          @(negedge `SPC6.l2clk);

// // debug!!!!!!!!!!!!!!!!!!!!!!!!!!!
// if (`SPC6.tlu.fls0.ptrap_flush[0] == 1) begin
//   `PR_ERROR ("dbg_chk", `ERROR, "watchStep0: TID 0 have tlu_flush_ifu !!!!!!!!!!!!!!!!!!!!!!!");
// end
// 

        if ((ssMode && `SPC6.tlu.fls0.ptrap_flush[0] == 0) || doMode) begin
    ...
    begin : FORK5_0
      if (doMode) begin
        ...
        while (`SPC6.pku_quiesce[0] == 0 ||
               `SPC6.ftu_ifu_quiesce[0] == 0 ||
               `SPC6.lsu_stb_empty[0] == 0) @(negedge `SPC6.l2clk);
...
//         while (ssMode && !`SPC6.spc_ss_complete) begin
//           // detect a second inst valid w/o a flush between
//           if (`SPC6.tlu_trap_pc_0_valid && `SPC6.tlu_trap_0_tid[1:0] == 0 && `SPC6.tlu.tlu_retry[0] == 1) begin
...
begin
  ...
  if (! `TOP.cpu.tcu_do_mode[6]) disable watchDOsteps0;

  // now wait for a thread to start. TCU internal signal.
  if (`SPC6.tlu.tlu_core_running_status[0] == 0)
    @(posedge `SPC6.tlu.tlu_core_running_status[0]);

  `PR_DEBUG("dbg_chk", `DEBUG, "watchDOsteps0 TID 0 unparked and running in DO mode!");

  // loop on step checking. DO mode is self stepping
  while (doMode) begin : watchDOsteps0loop// && `SPC6.tlu.tlu_core_running_status[7:0] != 0) begin
...
// got park request. transition into parking.  parking check.
always @(negedge `SPC6.tlu.tcu_core_running[1]) begin: GOTPARK1
  `PR_DEBUG("dbg_chk", `DEBUG, "TID 1 calling parkingCheck");
  if (enabled) parkingCheck1();
end

// got unpark request. transition into running. unparking check.
always @(posedge `SPC6.tlu.tcu_core_running[1]) begin: UNPARKCHECK1
  if (enabled && !ssMode) begin
...
// check for unexpected running transition
  always @(`SPC6.tlu.tlu_core_running_status[1]) begin: UNEXPECTED1
...
begin
  ...
  // need to see at least one assertion of flush
  // if (`SPC6.tlu_flush_ifu[1] !== 1) 
  //   @(posedge `SPC6.tlu_flush_ifu[1]); // has glitches
  if (`SPC6.ifu_ftu.ftu_agc_ctl.tlu_flush_ifu_f[1] !== 1)
    @(posedge `SPC6.ifu_ftu.ftu_agc_ctl.tlu_flush_ifu_f[1]);

  // need to see all of these go idle
  while (`SPC6.pku_quiesce[1] == 0 ||
         `SPC6.ftu_ifu_quiesce[1] == 0 ||
         `SPC6.lsu_stb_empty[1] == 0) 
     @(negedge `SPC6.l2clk);
...
begin //{
  ...
  while ((parkTrans[1] == 0 && ssMode == 0) || 
         (virtualPark[1] == 1 && ssMode == 1 && 
          `TOP.cpu.tcu_ss_request[6] == 0)) begin //{
    `PR_DEBUG ("dbg_chk", `DEBUG, "TID 1 watching for activity while parked/between steps. trans=%0h, virtualPark=%0h, tcu_ss_request=%0h",parkTrans[1],virtualPark[1],`TOP.cpu.tcu_ss_request[6]);
    ...
    // (core_running_status=0). This looks like an un-asked for step so it
    // has to be detected and ignored. Try stalling here during it.
    while (`SPC6.tlu.tcu_core_running[1] == 0 && 
           `SPC6.tlu.tlu_core_running_status[1] == 1) 
      @(negedge `SPC6.l2clk);

    // watch for instructions sneaking in
    // dec_inst_valid_m[1:0], one bit per group
    if (`SPC6.tlu_trap_pc_0_valid && `SPC6.tlu_trap_0_tid[1:0] == 1 && `SPC6.tlu.tlu_retry[0] == 1)
      `PR_ERROR ("dbg_chk", `ERROR, "ERROR: TID 1 had tlu_trap_pc_0_valid activity while parked/between steps!");

    if (`SPC6.dec_inst_valid_m[0] == 1 && `SPC6.dec_tid0_m[1:0] == 1)
      `PR_ERROR ("dbg_chk", `ERROR, "ERROR: TID 1 had dec_inst_valid_m activity while parked/between steps!");


  end //} while

  `PR_DEBUG ("dbg_chk", `DEBUG, "TID 1 STOP watching for activity while parked/between steps. trans=%0h, virtualPark=%0h, tcu_ss_request=%0h",parkTrans[1],virtualPark[1],`TOP.cpu.tcu_ss_request[6]);
...
begin
  ...
  // must see `SPC6.tlu.core_running_status[1] w/in count clocks
  count1 = 0;

  while (`SPC6.tlu.tlu_core_running_status[1] == 0) begin
    @(negedge `SPC6.l2clk);
  ...
  if (!doMode && !ssMode) begin
    ...
    // happened at once so while loop can be skipped.
      while (! (`SPC6.tlu_trap_pc_0_valid && 
                `SPC6.tlu_trap_0_tid[1:0] == 1)) begin
        @(negedge `SPC6.l2clk);
...
//     // look for SPU going active    
//     begin : FORK11_1
//       while (!(`SPC6.spu_tlu_cwq_busy || (`SPC6.spu.mct.spu_pmu_ma_busy[3] && `SPC6.spu.mct.spu_pmu_ma_busy[2:0] == 1)))
//         @(negedge `SPC6.l2clk);
...
//     end
    ...
    begin : FORK9_1
      // start step. tid wakes up/redirects and does "one instruction"
      while (!(`SPC6.tlu_trap_pc_0_valid && `SPC6.tlu_trap_0_tid[1:0] == 1 && `SPC6.tlu.tlu_retry[0] == 1)) begin
        // might see  `SPC6.tlu.mmu_reload_done[1] here if previous
        // redirect was due to a MMU miss. If so, decrement flush count because
        // the previous flush on prev pc_valid "does not count" and there can be
        // any number of them w/ no way to predict quantity.
        if (`SPC6.tlu.mmu_reload_done[1] && ssMode) begin
      ...
      @(negedge `SPC6.pku_quiesce[1]);    
      `PR_DEBUG("dbg_chk", `DEBUG, "watchStep1: TID 1 got negedge pku_quiesce, step waiting on dec_inst_valid_m!");
    
      while (!(`SPC6.dec_inst_valid_m[0] == 1 && `SPC6.dec_tid0_m[1:0] == 1)) @(negedge `SPC6.l2clk);
      ...
      //       if (!`SPC6.spu_tlu_cwq_busy && 
//           !(`SPC6.spu.mct.spu_pmu_ma_busy[3] && 
//             `SPC6.spu.mct.spu_pmu_ma_busy[2:0] == 1)) spuActive[1] = 0;
    end
  join
...
//     begin : FORK12_1
//       while (!(`SPC6.spu_tlu_cwq_busy || 
//         ((`SPC6.spu.mct.spu_pmu_ma_busy[3] && `SPC6.spu.mct.spu_pmu_ma_busy[2:0] == 1 && doMode) || (`SPC6.spu.mct.spu_pmu_ma_busy[3] && ssMode))))
//         @(negedge `SPC6.l2clk);
//       spuActive[1] = 1;
//     end
    ...
    begin : FORK6_1
        // checking ptrap_flush to make sure that flush is not caused by lsu_trap_flush
        while (`SPC6.tlu_flush_ifu[1] == 0)
          @(negedge `SPC6.l2clk);

// // debug!!!!!!!!!!!!!!!!!!!!!!!!!!!
// if (`SPC6.tlu.fls0.ptrap_flush[1] == 1) begin
//   `PR_ERROR ("dbg_chk", `ERROR, "watchStep1: TID 1 have tlu_flush_ifu !!!!!!!!!!!!!!!!!!!!!!!");
// end
// 

        if ((ssMode && `SPC6.tlu.fls0.ptrap_flush[1] == 0) || doMode) begin
    ...
    begin : FORK5_1
      if (doMode) begin
        ...
        while (`SPC6.pku_quiesce[1] == 0 ||
               `SPC6.ftu_ifu_quiesce[1] == 0 ||
               `SPC6.lsu_stb_empty[1] == 0) @(negedge `SPC6.l2clk);
...
//         while (ssMode && !`SPC6.spc_ss_complete) begin
//           // detect a second inst valid w/o a flush between
//           if (`SPC6.tlu_trap_pc_0_valid && `SPC6.tlu_trap_0_tid[1:0] == 1 && `SPC6.tlu.tlu_retry[0] == 1) begin
//             // delay error so flush thread can kill this in the "got flush" case
//             @(negedge `SPC6.l2clk); 
...
begin
  ...
  if (! `TOP.cpu.tcu_do_mode[6]) disable watchDOsteps1;

  // now wait for a thread to start. TCU internal signal.
  if (`SPC6.tlu.tlu_core_running_status[1] == 0)
    @(posedge `SPC6.tlu.tlu_core_running_status[1]);

  `PR_DEBUG("dbg_chk", `DEBUG, "watchDOsteps1 TID 1 unparked and running in DO mode!");

  // loop on step checking. DO mode is self stepping
  while (doMode) begin : watchDOsteps1loop// && `SPC6.tlu.tlu_core_running_status[7:0] != 0) begin
...
// got park request. transition into parking.  parking check.
always @(negedge `SPC6.tlu.tcu_core_running[2]) begin: GOTPARK2
  `PR_DEBUG("dbg_chk", `DEBUG, "TID 2 calling parkingCheck");
  if (enabled) parkingCheck2();
end

// got unpark request. transition into running. unparking check.
always @(posedge `SPC6.tlu.tcu_core_running[2]) begin: UNPARKCHECK2
  if (enabled && !ssMode) begin
...
// check for unexpected running transition
  always @(`SPC6.tlu.tlu_core_running_status[2]) begin: UNEXPECTED2
...
begin
  ...
  // need to see at least one assertion of flush
  // if (`SPC6.tlu_flush_ifu[2] !== 1) 
  //   @(posedge `SPC6.tlu_flush_ifu[2]); // has glitches
  if (`SPC6.ifu_ftu.ftu_agc_ctl.tlu_flush_ifu_f[2] !== 1)
    @(posedge `SPC6.ifu_ftu.ftu_agc_ctl.tlu_flush_ifu_f[2]);

  // need to see all of these go idle
  while (`SPC6.pku_quiesce[2] == 0 ||
         `SPC6.ftu_ifu_quiesce[2] == 0 ||
         `SPC6.lsu_stb_empty[2] == 0) 
     @(negedge `SPC6.l2clk);
...
begin //{
  ...
  while ((parkTrans[2] == 0 && ssMode == 0) || 
         (virtualPark[2] == 1 && ssMode == 1 && 
          `TOP.cpu.tcu_ss_request[6] == 0)) begin //{
    `PR_DEBUG ("dbg_chk", `DEBUG, "TID 2 watching for activity while parked/between steps. trans=%0h, virtualPark=%0h, tcu_ss_request=%0h",parkTrans[2],virtualPark[2],`TOP.cpu.tcu_ss_request[6]);
    ...
    // (core_running_status=0). This looks like an un-asked for step so it
    // has to be detected and ignored. Try stalling here during it.
    while (`SPC6.tlu.tcu_core_running[2] == 0 && 
           `SPC6.tlu.tlu_core_running_status[2] == 1) 
      @(negedge `SPC6.l2clk);

    // watch for instructions sneaking in
    // dec_inst_valid_m[1:0], one bit per group
    if (`SPC6.tlu_trap_pc_0_valid && `SPC6.tlu_trap_0_tid[1:0] == 2 && `SPC6.tlu.tlu_retry[0] == 1)
      `PR_ERROR ("dbg_chk", `ERROR, "ERROR: TID 2 had tlu_trap_pc_0_valid activity while parked/between steps!");

    if (`SPC6.dec_inst_valid_m[0] == 1 && `SPC6.dec_tid0_m[1:0] == 2)
      `PR_ERROR ("dbg_chk", `ERROR, "ERROR: TID 2 had dec_inst_valid_m activity while parked/between steps!");


  end //} while

  `PR_DEBUG ("dbg_chk", `DEBUG, "TID 2 STOP watching for activity while parked/between steps. trans=%0h, virtualPark=%0h, tcu_ss_request=%0h",parkTrans[2],virtualPark[2],`TOP.cpu.tcu_ss_request[6]);
...
begin
  ...
  // must see `SPC6.tlu.core_running_status[2] w/in count clocks
  count2 = 0;

  while (`SPC6.tlu.tlu_core_running_status[2] == 0) begin
    @(negedge `SPC6.l2clk);
  ...
  if (!doMode && !ssMode) begin
    ...
    // happened at once so while loop can be skipped.
      while (! (`SPC6.tlu_trap_pc_0_valid && 
                `SPC6.tlu_trap_0_tid[1:0] == 2)) begin
        @(negedge `SPC6.l2clk);
...
//     // look for SPU going active    
//     begin : FORK11_2
//       while (!(`SPC6.spu_tlu_cwq_busy || (`SPC6.spu.mct.spu_pmu_ma_busy[3] && `SPC6.spu.mct.spu_pmu_ma_busy[2:0] == 2)))
//         @(negedge `SPC6.l2clk);
...
//     end
    ...
    begin : FORK9_2
      // start step. tid wakes up/redirects and does "one instruction"
      while (!(`SPC6.tlu_trap_pc_0_valid && `SPC6.tlu_trap_0_tid[1:0] == 2 && `SPC6.tlu.tlu_retry[0] == 1)) begin
        // might see  `SPC6.tlu.mmu_reload_done[2] here if previous
        // redirect was due to a MMU miss. If so, decrement flush count because
        // the previous flush on prev pc_valid "does not count" and there can be
        // any number of them w/ no way to predict quantity.
        if (`SPC6.tlu.mmu_reload_done[2] && ssMode) begin
      ...
      @(negedge `SPC6.pku_quiesce[2]);    
      `PR_DEBUG("dbg_chk", `DEBUG, "watchStep2: TID 2 got negedge pku_quiesce, step waiting on dec_inst_valid_m!");
    
      while (!(`SPC6.dec_inst_valid_m[0] == 1 && `SPC6.dec_tid0_m[1:0] == 2)) @(negedge `SPC6.l2clk);
      ...
      //       if (!`SPC6.spu_tlu_cwq_busy && 
//           !(`SPC6.spu.mct.spu_pmu_ma_busy[3] && 
//             `SPC6.spu.mct.spu_pmu_ma_busy[2:0] == 2)) spuActive[2] = 0;
    end
  join
...
//     begin : FORK12_2
//       while (!(`SPC6.spu_tlu_cwq_busy || 
//         ((`SPC6.spu.mct.spu_pmu_ma_busy[3] && `SPC6.spu.mct.spu_pmu_ma_busy[2:0] == 2 && doMode) || (`SPC6.spu.mct.spu_pmu_ma_busy[3] && ssMode))))
//         @(negedge `SPC6.l2clk);
//       spuActive[2] = 1;
//     end
    ...
    begin : FORK6_2
        // checking ptrap_flush to make sure that flush is not caused by lsu_trap_flush
        while (`SPC6.tlu_flush_ifu[2] == 0)
          @(negedge `SPC6.l2clk);

// // debug!!!!!!!!!!!!!!!!!!!!!!!!!!!
// if (`SPC6.tlu.fls0.ptrap_flush[2] == 1) begin
//   `PR_ERROR ("dbg_chk", `ERROR, "watchStep2: TID 2 have tlu_flush_ifu !!!!!!!!!!!!!!!!!!!!!!!");
// end
// 

        if ((ssMode && `SPC6.tlu.fls0.ptrap_flush[2] == 0) || doMode) begin
    ...
    begin : FORK5_2
      if (doMode) begin
        ...
        while (`SPC6.pku_quiesce[2] == 0 ||
               `SPC6.ftu_ifu_quiesce[2] == 0 ||
               `SPC6.lsu_stb_empty[2] == 0) @(negedge `SPC6.l2clk);
...
//         while (ssMode && !`SPC6.spc_ss_complete) begin
//           // detect a second inst valid w/o a flush between
//           if (`SPC6.tlu_trap_pc_0_valid && `SPC6.tlu_trap_0_tid[1:0] == 2 && `SPC6.tlu.tlu_retry[0] == 1) begin
//             // delay error so flush thread can kill this in the "got flush" case
//             @(negedge `SPC6.l2clk); 
...
begin
  ...
  if (! `TOP.cpu.tcu_do_mode[6]) disable watchDOsteps2;

  // now wait for a thread to start. TCU internal signal.
  if (`SPC6.tlu.tlu_core_running_status[2] == 0)
    @(posedge `SPC6.tlu.tlu_core_running_status[2]);

  `PR_DEBUG("dbg_chk", `DEBUG, "watchDOsteps2 TID 2 unparked and running in DO mode!");

  // loop on step checking. DO mode is self stepping
  while (doMode) begin : watchDOsteps2loop// && `SPC6.tlu.tlu_core_running_status[7:0] != 0) begin
...
// got park request. transition into parking.  parking check.
always @(negedge `SPC6.tlu.tcu_core_running[3]) begin: GOTPARK3
  `PR_DEBUG("dbg_chk", `DEBUG, "TID 3 calling parkingCheck");
  if (enabled) parkingCheck3();
end

// got unpark request. transition into running. unparking check.
always @(posedge `SPC6.tlu.tcu_core_running[3]) begin: UNPARKCHECK3
  if (enabled && !ssMode) begin
...
// check for unexpected running transition
  always @(`SPC6.tlu.tlu_core_running_status[3]) begin: UNEXPECTED3
...
begin
  ...
  // need to see at least one assertion of flush
  // if (`SPC6.tlu_flush_ifu[3] !== 1) 
  //   @(posedge `SPC6.tlu_flush_ifu[3]); // has glitches
  if (`SPC6.ifu_ftu.ftu_agc_ctl.tlu_flush_ifu_f[3] !== 1)
    @(posedge `SPC6.ifu_ftu.ftu_agc_ctl.tlu_flush_ifu_f[3]);

  // need to see all of these go idle
  while (`SPC6.pku_quiesce[3] == 0 ||
         `SPC6.ftu_ifu_quiesce[3] == 0 ||
         `SPC6.lsu_stb_empty[3] == 0) 
     @(negedge `SPC6.l2clk);
...
begin //{
  ...
  while ((parkTrans[3] == 0 && ssMode == 0) || 
         (virtualPark[3] == 1 && ssMode == 1 && 
          `TOP.cpu.tcu_ss_request[6] == 0)) begin //{
    `PR_DEBUG ("dbg_chk", `DEBUG, "TID 3 watching for activity while parked/between steps. trans=%0h, virtualPark=%0h, tcu_ss_request=%0h",parkTrans[3],virtualPark[3],`TOP.cpu.tcu_ss_request[6]);
    ...
    // (core_running_status=0). This looks like an un-asked for step so it
    // has to be detected and ignored. Try stalling here during it.
    while (`SPC6.tlu.tcu_core_running[3] == 0 && 
           `SPC6.tlu.tlu_core_running_status[3] == 1) 
      @(negedge `SPC6.l2clk);

    // watch for instructions sneaking in
    // dec_inst_valid_m[1:0], one bit per group
    if (`SPC6.tlu_trap_pc_0_valid && `SPC6.tlu_trap_0_tid[1:0] == 3 && `SPC6.tlu.tlu_retry[0] == 1)
      `PR_ERROR ("dbg_chk", `ERROR, "ERROR: TID 3 had tlu_trap_pc_0_valid activity while parked/between steps!");

    if (`SPC6.dec_inst_valid_m[0] == 1 && `SPC6.dec_tid0_m[1:0] == 3)
      `PR_ERROR ("dbg_chk", `ERROR, "ERROR: TID 3 had dec_inst_valid_m activity while parked/between steps!");


  end //} while

  `PR_DEBUG ("dbg_chk", `DEBUG, "TID 3 STOP watching for activity while parked/between steps. trans=%0h, virtualPark=%0h, tcu_ss_request=%0h",parkTrans[3],virtualPark[3],`TOP.cpu.tcu_ss_request[6]);
...
begin
  ...
  // must see `SPC6.tlu.core_running_status[3] w/in count clocks
  count3 = 0;

  while (`SPC6.tlu.tlu_core_running_status[3] == 0) begin
    @(negedge `SPC6.l2clk);
  ...
  if (!doMode && !ssMode) begin
    ...
    // happened at once so while loop can be skipped.
      while (! (`SPC6.tlu_trap_pc_0_valid && 
                `SPC6.tlu_trap_0_tid[1:0] == 3)) begin
        @(negedge `SPC6.l2clk);
...
//     // look for SPU going active    
//     begin : FORK11_3
//       while (!(`SPC6.spu_tlu_cwq_busy || (`SPC6.spu.mct.spu_pmu_ma_busy[3] && `SPC6.spu.mct.spu_pmu_ma_busy[2:0] == 3)))
//         @(negedge `SPC6.l2clk);
...
//     end
    ...
    begin : FORK9_3
      // start step. tid wakes up/redirects and does "one instruction"
      while (!(`SPC6.tlu_trap_pc_0_valid && `SPC6.tlu_trap_0_tid[1:0] == 3 && `SPC6.tlu.tlu_retry[0] == 1)) begin
        // might see  `SPC6.tlu.mmu_reload_done[3] here if previous
        // redirect was due to a MMU miss. If so, decrement flush count because
        // the previous flush on prev pc_valid "does not count" and there can be
        // any number of them w/ no way to predict quantity.
        if (`SPC6.tlu.mmu_reload_done[3] && ssMode) begin
      ...
      @(negedge `SPC6.pku_quiesce[3]);    
      `PR_DEBUG("dbg_chk", `DEBUG, "watchStep3: TID 3 got negedge pku_quiesce, step waiting on dec_inst_valid_m!");
    
      while (!(`SPC6.dec_inst_valid_m[0] == 1 && `SPC6.dec_tid0_m[1:0] == 3)) @(negedge `SPC6.l2clk);
      ...
      //       if (!`SPC6.spu_tlu_cwq_busy && 
//           !(`SPC6.spu.mct.spu_pmu_ma_busy[3] && 
//             `SPC6.spu.mct.spu_pmu_ma_busy[2:0] == 3)) spuActive[3] = 0;
    end
  join
...
//     begin : FORK12_3
//       while (!(`SPC6.spu_tlu_cwq_busy || 
//         ((`SPC6.spu.mct.spu_pmu_ma_busy[3] && `SPC6.spu.mct.spu_pmu_ma_busy[2:0] == 3 && doMode) || (`SPC6.spu.mct.spu_pmu_ma_busy[3] && ssMode))))
//         @(negedge `SPC6.l2clk);
//       spuActive[3] = 1;
//     end
    ...
    begin : FORK6_3
        // checking ptrap_flush to make sure that flush is not caused by lsu_trap_flush
        while (`SPC6.tlu_flush_ifu[3] == 0)
          @(negedge `SPC6.l2clk);

// // debug!!!!!!!!!!!!!!!!!!!!!!!!!!!
// if (`SPC6.tlu.fls0.ptrap_flush[3] == 1) begin
//   `PR_ERROR ("dbg_chk", `ERROR, "watchStep3: TID 3 have tlu_flush_ifu !!!!!!!!!!!!!!!!!!!!!!!");
// end
// 

        if ((ssMode && `SPC6.tlu.fls0.ptrap_flush[3] == 0) || doMode) begin
    ...
    begin : FORK5_3
      if (doMode) begin
        ...
        while (`SPC6.pku_quiesce[3] == 0 ||
               `SPC6.ftu_ifu_quiesce[3] == 0 ||
               `SPC6.lsu_stb_empty[3] == 0) @(negedge `SPC6.l2clk);
...
//         while (ssMode && !`SPC6.spc_ss_complete) begin
//           // detect a second inst valid w/o a flush between
//           if (`SPC6.tlu_trap_pc_0_valid && `SPC6.tlu_trap_0_tid[1:0] == 3 && `SPC6.tlu.tlu_retry[0] == 1) begin
//             // delay error so flush thread can kill this in the "got flush" case
//             @(negedge `SPC6.l2clk); 
...
begin
  ...
  if (! `TOP.cpu.tcu_do_mode[6]) disable watchDOsteps3;

  // now wait for a thread to start. TCU internal signal.
  if (`SPC6.tlu.tlu_core_running_status[3] == 0)
    @(posedge `SPC6.tlu.tlu_core_running_status[3]);

  `PR_DEBUG("dbg_chk", `DEBUG, "watchDOsteps3 TID 3 unparked and running in DO mode!");

  // loop on step checking. DO mode is self stepping
  while (doMode) begin : watchDOsteps3loop// && `SPC6.tlu.tlu_core_running_status[7:0] != 0) begin
...
// got park request. transition into parking.  parking check.
always @(negedge `SPC6.tlu.tcu_core_running[4]) begin: GOTPARK4
  `PR_DEBUG("dbg_chk", `DEBUG, "TID 4 calling parkingCheck");
  if (enabled) parkingCheck4();
end

// got unpark request. transition into running. unparking check.
always @(posedge `SPC6.tlu.tcu_core_running[4]) begin: UNPARKCHECK4
  if (enabled && !ssMode) begin
...
// check for unexpected running transition
  always @(`SPC6.tlu.tlu_core_running_status[4]) begin: UNEXPECTED4
...
begin
  ...
  // need to see at least one assertion of flush
  // if (`SPC6.tlu_flush_ifu[4] !== 1) 
  //   @(posedge `SPC6.tlu_flush_ifu[4]); // has glitches
  if (`SPC6.ifu_ftu.ftu_agc_ctl.tlu_flush_ifu_f[4] !== 1)
    @(posedge `SPC6.ifu_ftu.ftu_agc_ctl.tlu_flush_ifu_f[4]);

  // need to see all of these go idle
  while (`SPC6.pku_quiesce[4] == 0 ||
         `SPC6.ftu_ifu_quiesce[4] == 0 ||
         `SPC6.lsu_stb_empty[4] == 0) 
     @(negedge `SPC6.l2clk);
...
begin //{
  ...
  while ((parkTrans[4] == 0 && ssMode == 0) || 
         (virtualPark[4] == 1 && ssMode == 1 && 
          `TOP.cpu.tcu_ss_request[6] == 0)) begin //{
    `PR_DEBUG ("dbg_chk", `DEBUG, "TID 4 watching for activity while parked/between steps. trans=%0h, virtualPark=%0h, tcu_ss_request=%0h",parkTrans[4],virtualPark[4],`TOP.cpu.tcu_ss_request[6]);
    ...
    // (core_running_status=0). This looks like an un-asked for step so it
    // has to be detected and ignored. Try stalling here during it.
    while (`SPC6.tlu.tcu_core_running[4] == 0 && 
           `SPC6.tlu.tlu_core_running_status[4] == 1) 
      @(negedge `SPC6.l2clk);

    // watch for instructions sneaking in
    // dec_inst_valid_m[1:0], one bit per group
    if (`SPC6.tlu_trap_pc_1_valid && `SPC6.tlu_trap_1_tid[1:0] == 4 && `SPC6.tlu.tlu_retry[1] == 1)
      `PR_ERROR ("dbg_chk", `ERROR, "ERROR: TID 4 had tlu_trap_pc_1_valid activity while parked/between steps!");

    if (`SPC6.dec_inst_valid_m[1] == 1 && `SPC6.dec_tid1_m[1:0] == 0)
      `PR_ERROR ("dbg_chk", `ERROR, "ERROR: TID 4 had dec_inst_valid_m activity while parked/between steps!");


  end //} while

  `PR_DEBUG ("dbg_chk", `DEBUG, "TID 4 STOP watching for activity while parked/between steps. trans=%0h, virtualPark=%0h, tcu_ss_request=%0h",parkTrans[4],virtualPark[4],`TOP.cpu.tcu_ss_request[6]);
...
begin
  ...
  // must see `SPC6.tlu.core_running_status[4] w/in count clocks
  count4 = 0;

  while (`SPC6.tlu.tlu_core_running_status[4] == 0) begin
    @(negedge `SPC6.l2clk);
  ...
  if (!doMode && !ssMode) begin
    ...
    // if this is true, the redirect and the assertion of core_running_status
    // happened at once so while loop can be skipped.
      while (! (`SPC6.tlu_trap_pc_1_valid && 
                `SPC6.tlu_trap_1_tid[1:0] == 0)) begin
        @(negedge `SPC6.l2clk);
...
//     // look for SPU going active    
//     begin : FORK11_4
//       while (!(`SPC6.spu_tlu_cwq_busy || (`SPC6.spu.mct.spu_pmu_ma_busy[3] && `SPC6.spu.mct.spu_pmu_ma_busy[2:0] == 4)))
//         @(negedge `SPC6.l2clk);
...
//     end
    ...
    begin : FORK9_4
      // start step. tid wakes up/redirects and does "one instruction"
      while (!(`SPC6.tlu_trap_pc_1_valid && `SPC6.tlu_trap_1_tid[1:0] == 0 && `SPC6.tlu.tlu_retry[1] == 1)) begin
        // might see  `SPC6.tlu.mmu_reload_done[4] here if previous
        // redirect was due to a MMU miss. If so, decrement flush count because
        // the previous flush on prev pc_valid "does not count" and there can be
        // any number of them w/ no way to predict quantity.
        if (`SPC6.tlu.mmu_reload_done[4] && ssMode) begin
      ...
      @(negedge `SPC6.pku_quiesce[4]);    
      `PR_DEBUG("dbg_chk", `DEBUG, "watchStep4: TID 4 got negedge pku_quiesce, step waiting on dec_inst_valid_m!");
    
      while (!(`SPC6.dec_inst_valid_m[1] == 1 && `SPC6.dec_tid1_m[1:0] == 0)) @(negedge `SPC6.l2clk);
      ...
      //       if (!`SPC6.spu_tlu_cwq_busy && 
//           !(`SPC6.spu.mct.spu_pmu_ma_busy[3] && 
//             `SPC6.spu.mct.spu_pmu_ma_busy[2:0] == 4)) spuActive[4] = 0;
    end
  join
...
//     begin : FORK12_4
//       while (!(`SPC6.spu_tlu_cwq_busy || 
//         ((`SPC6.spu.mct.spu_pmu_ma_busy[3] && `SPC6.spu.mct.spu_pmu_ma_busy[2:0] == 4 && doMode) || (`SPC6.spu.mct.spu_pmu_ma_busy[3] && ssMode))))
//         @(negedge `SPC6.l2clk);
//       spuActive[4] = 1;
//     end
    ...
    begin : FORK6_4
        // checking ptrap_flush to make sure that flush is not caused by lsu_trap_flush
        while (`SPC6.tlu_flush_ifu[4] == 0)
          @(negedge `SPC6.l2clk);

// // debug!!!!!!!!!!!!!!!!!!!!!!!!!!!
// if (`SPC6.tlu.fls1.ptrap_flush[0] == 1) begin
//   `PR_ERROR ("dbg_chk", `ERROR, "watchStep4: TID 4 have tlu_flush_ifu !!!!!!!!!!!!!!!!!!!!!!!");
// end
// 

        if ((ssMode && `SPC6.tlu.fls1.ptrap_flush[0] == 0) || doMode) begin
    ...
    begin : FORK5_4
      if (doMode) begin
        ...
        while (`SPC6.pku_quiesce[4] == 0 ||
               `SPC6.ftu_ifu_quiesce[4] == 0 ||
               `SPC6.lsu_stb_empty[4] == 0) @(negedge `SPC6.l2clk);
...
//         while (ssMode && !`SPC6.spc_ss_complete) begin
//           // detect a second inst valid w/o a flush between
//           if (`SPC6.tlu_trap_pc_1_valid && `SPC6.tlu_trap_1_tid[1:0] == 0 && `SPC6.tlu.tlu_retry[1] == 1) begin
//             // delay error so flush thread can kill this in the "got flush" case
//             @(negedge `SPC6.l2clk); 
...
begin
  ...
  if (! `TOP.cpu.tcu_do_mode[6]) disable watchDOsteps4;

  // now wait for a thread to start. TCU internal signal.
  if (`SPC6.tlu.tlu_core_running_status[4] == 0)
    @(posedge `SPC6.tlu.tlu_core_running_status[4]);

  `PR_DEBUG("dbg_chk", `DEBUG, "watchDOsteps4 TID 4 unparked and running in DO mode!");

  // loop on step checking. DO mode is self stepping
  while (doMode) begin : watchDOsteps4loop// && `SPC6.tlu.tlu_core_running_status[7:0] != 0) begin
...
// got park request. transition into parking.  parking check.
always @(negedge `SPC6.tlu.tcu_core_running[5]) begin: GOTPARK5
  `PR_DEBUG("dbg_chk", `DEBUG, "TID 5 calling parkingCheck");
  if (enabled) parkingCheck5();
end

// got unpark request. transition into running. unparking check.
always @(posedge `SPC6.tlu.tcu_core_running[5]) begin: UNPARKCHECK5
  if (enabled && !ssMode) begin
...
// check for unexpected running transition
  always @(`SPC6.tlu.tlu_core_running_status[5]) begin: UNEXPECTED5
...
begin
  ...
  // need to see at least one assertion of flush
  // if (`SPC6.tlu_flush_ifu[5] !== 1) 
  //   @(posedge `SPC6.tlu_flush_ifu[5]); // has glitches
  if (`SPC6.ifu_ftu.ftu_agc_ctl.tlu_flush_ifu_f[5] !== 1)
    @(posedge `SPC6.ifu_ftu.ftu_agc_ctl.tlu_flush_ifu_f[5]);

  // need to see all of these go idle
  while (`SPC6.pku_quiesce[5] == 0 ||
         `SPC6.ftu_ifu_quiesce[5] == 0 ||
         `SPC6.lsu_stb_empty[5] == 0) 
     @(negedge `SPC6.l2clk);
...
begin //{
  ...
  while ((parkTrans[5] == 0 && ssMode == 0) || 
         (virtualPark[5] == 1 && ssMode == 1 && 
          `TOP.cpu.tcu_ss_request[6] == 0)) begin //{
    `PR_DEBUG ("dbg_chk", `DEBUG, "TID 5 watching for activity while parked/between steps. trans=%0h, virtualPark=%0h, tcu_ss_request=%0h",parkTrans[5],virtualPark[5],`TOP.cpu.tcu_ss_request[6]);
    ...
    // (core_running_status=0). This looks like an un-asked for step so it
    // has to be detected and ignored. Try stalling here during it.
    while (`SPC6.tlu.tcu_core_running[5] == 0 && 
           `SPC6.tlu.tlu_core_running_status[5] == 1) 
      @(negedge `SPC6.l2clk);

    // watch for instructions sneaking in
    // dec_inst_valid_m[1:0], one bit per group
    if (`SPC6.tlu_trap_pc_1_valid && `SPC6.tlu_trap_1_tid[1:0] == 5 && `SPC6.tlu.tlu_retry[1] == 1)
      `PR_ERROR ("dbg_chk", `ERROR, "ERROR: TID 5 had tlu_trap_pc_1_valid activity while parked/between steps!");

    if (`SPC6.dec_inst_valid_m[1] == 1 && `SPC6.dec_tid1_m[1:0] == 1)
      `PR_ERROR ("dbg_chk", `ERROR, "ERROR: TID 5 had dec_inst_valid_m activity while parked/between steps!");


  end //} while

  `PR_DEBUG ("dbg_chk", `DEBUG, "TID 5 STOP watching for activity while parked/between steps. trans=%0h, virtualPark=%0h, tcu_ss_request=%0h",parkTrans[5],virtualPark[5],`TOP.cpu.tcu_ss_request[6]);
...
begin
  ...
  // must see `SPC6.tlu.core_running_status[5] w/in count clocks
  count5 = 0;

  while (`SPC6.tlu.tlu_core_running_status[5] == 0) begin
    @(negedge `SPC6.l2clk);
  ...
  if (!doMode && !ssMode) begin
    ...
    // happened at once so while loop can be skipped.
      while (! (`SPC6.tlu_trap_pc_1_valid && 
                `SPC6.tlu_trap_1_tid[1:0] == 1)) begin
        @(negedge `SPC6.l2clk);
...
//     // look for SPU going active    
//     begin : FORK11_5
//       while (!(`SPC6.spu_tlu_cwq_busy || (`SPC6.spu.mct.spu_pmu_ma_busy[3] && `SPC6.spu.mct.spu_pmu_ma_busy[2:0] == 5)))
//         @(negedge `SPC6.l2clk);
...
//     end
    ...
    begin : FORK9_5
      // start step. tid wakes up/redirects and does "one instruction"
      while (!(`SPC6.tlu_trap_pc_1_valid && `SPC6.tlu_trap_1_tid[1:0] == 1 && `SPC6.tlu.tlu_retry[1] == 1)) begin
        // might see  `SPC6.tlu.mmu_reload_done[5] here if previous
        // redirect was due to a MMU miss. If so, decrement flush count because
        // the previous flush on prev pc_valid "does not count" and there can be
        // any number of them w/ no way to predict quantity.
        if (`SPC6.tlu.mmu_reload_done[5] && ssMode) begin
      ...
      @(negedge `SPC6.pku_quiesce[5]);    
      `PR_DEBUG("dbg_chk", `DEBUG, "watchStep5: TID 5 got negedge pku_quiesce, step waiting on dec_inst_valid_m!");
    
      while (!(`SPC6.dec_inst_valid_m[1] == 1 && `SPC6.dec_tid1_m[1:0] == 1)) @(negedge `SPC6.l2clk);
      ...
      //       if (!`SPC6.spu_tlu_cwq_busy && 
//           !(`SPC6.spu.mct.spu_pmu_ma_busy[3] && 
//             `SPC6.spu.mct.spu_pmu_ma_busy[2:0] == 5)) spuActive[5] = 0;
    end
  join
...
//     begin : FORK12_5
//       while (!(`SPC6.spu_tlu_cwq_busy || 
//         ((`SPC6.spu.mct.spu_pmu_ma_busy[3] && `SPC6.spu.mct.spu_pmu_ma_busy[2:0] == 5 && doMode) || (`SPC6.spu.mct.spu_pmu_ma_busy[3] && ssMode))))
//         @(negedge `SPC6.l2clk);
//       spuActive[5] = 1;
//     end
    ...
    begin : FORK6_5
        // checking ptrap_flush to make sure that flush is not caused by lsu_trap_flush
        while (`SPC6.tlu_flush_ifu[5] == 0)
          @(negedge `SPC6.l2clk);

// // debug!!!!!!!!!!!!!!!!!!!!!!!!!!!
// if (`SPC6.tlu.fls1.ptrap_flush[1] == 1) begin
//   `PR_ERROR ("dbg_chk", `ERROR, "watchStep5: TID 5 have tlu_flush_ifu !!!!!!!!!!!!!!!!!!!!!!!");
// end
// 

        if ((ssMode && `SPC6.tlu.fls1.ptrap_flush[1] == 0) || doMode) begin
    ...
    begin : FORK5_5
      if (doMode) begin
        ...
        while (`SPC6.pku_quiesce[5] == 0 ||
               `SPC6.ftu_ifu_quiesce[5] == 0 ||
               `SPC6.lsu_stb_empty[5] == 0) @(negedge `SPC6.l2clk);
...
//         while (ssMode && !`SPC6.spc_ss_complete) begin
//           // detect a second inst valid w/o a flush between
//           if (`SPC6.tlu_trap_pc_1_valid && `SPC6.tlu_trap_1_tid[1:0] == 1 && `SPC6.tlu.tlu_retry[1] == 1) begin
//             // delay error so flush thread can kill this in the "got flush" case
//             @(negedge `SPC6.l2clk); 
...
begin
  ...
  if (! `TOP.cpu.tcu_do_mode[6]) disable watchDOsteps5;

  // now wait for a thread to start. TCU internal signal.
  if (`SPC6.tlu.tlu_core_running_status[5] == 0)
    @(posedge `SPC6.tlu.tlu_core_running_status[5]);

  `PR_DEBUG("dbg_chk", `DEBUG, "watchDOsteps5 TID 5 unparked and running in DO mode!");

  // loop on step checking. DO mode is self stepping
  while (doMode) begin : watchDOsteps5loop// && `SPC6.tlu.tlu_core_running_status[7:0] != 0) begin
...
// got park request. transition into parking.  parking check.
always @(negedge `SPC6.tlu.tcu_core_running[6]) begin: GOTPARK6
  `PR_DEBUG("dbg_chk", `DEBUG, "TID 6 calling parkingCheck");
  if (enabled) parkingCheck6();
end

// got unpark request. transition into running. unparking check.
always @(posedge `SPC6.tlu.tcu_core_running[6]) begin: UNPARKCHECK6
  if (enabled && !ssMode) begin
...
// check for unexpected running transition
  always @(`SPC6.tlu.tlu_core_running_status[6]) begin: UNEXPECTED6
...
begin
  ...
  // need to see at least one assertion of flush
  // if (`SPC6.tlu_flush_ifu[6] !== 1) 
  //   @(posedge `SPC6.tlu_flush_ifu[6]); // has glitches
  if (`SPC6.ifu_ftu.ftu_agc_ctl.tlu_flush_ifu_f[6] !== 1)
    @(posedge `SPC6.ifu_ftu.ftu_agc_ctl.tlu_flush_ifu_f[6]);

  // need to see all of these go idle
  while (`SPC6.pku_quiesce[6] == 0 ||
         `SPC6.ftu_ifu_quiesce[6] == 0 ||
         `SPC6.lsu_stb_empty[6] == 0) 
     @(negedge `SPC6.l2clk);
...
begin //{
  ...
  while ((parkTrans[6] == 0 && ssMode == 0) || 
         (virtualPark[6] == 1 && ssMode == 1 && 
          `TOP.cpu.tcu_ss_request[6] == 0)) begin //{
    `PR_DEBUG ("dbg_chk", `DEBUG, "TID 6 watching for activity while parked/between steps. trans=%0h, virtualPark=%0h, tcu_ss_request=%0h",parkTrans[6],virtualPark[6],`TOP.cpu.tcu_ss_request[6]);
    ...
    // (core_running_status=0). This looks like an un-asked for step so it
    // has to be detected and ignored. Try stalling here during it.
    while (`SPC6.tlu.tcu_core_running[6] == 0 && 
           `SPC6.tlu.tlu_core_running_status[6] == 1) 
      @(negedge `SPC6.l2clk);

    // watch for instructions sneaking in
    // dec_inst_valid_m[1:0], one bit per group
    if (`SPC6.tlu_trap_pc_1_valid && `SPC6.tlu_trap_1_tid[1:0] == 6 && `SPC6.tlu.tlu_retry[1] == 1)
      `PR_ERROR ("dbg_chk", `ERROR, "ERROR: TID 6 had tlu_trap_pc_1_valid activity while parked/between steps!");

    if (`SPC6.dec_inst_valid_m[1] == 1 && `SPC6.dec_tid1_m[1:0] == 2)
      `PR_ERROR ("dbg_chk", `ERROR, "ERROR: TID 6 had dec_inst_valid_m activity while parked/between steps!");


  end //} while

  `PR_DEBUG ("dbg_chk", `DEBUG, "TID 6 STOP watching for activity while parked/between steps. trans=%0h, virtualPark=%0h, tcu_ss_request=%0h",parkTrans[6],virtualPark[6],`TOP.cpu.tcu_ss_request[6]);
...
begin
  ...
  // must see `SPC6.tlu.core_running_status[6] w/in count clocks
  count6 = 0;

  while (`SPC6.tlu.tlu_core_running_status[6] == 0) begin
    @(negedge `SPC6.l2clk);
  ...
  if (!doMode && !ssMode) begin
    ...
    // happened at once so while loop can be skipped.
      while (! (`SPC6.tlu_trap_pc_1_valid && 
                `SPC6.tlu_trap_1_tid[1:0] == 2)) begin
        @(negedge `SPC6.l2clk);
...
//     // look for SPU going active    
//     begin : FORK11_6
//       while (!(`SPC6.spu_tlu_cwq_busy || (`SPC6.spu.mct.spu_pmu_ma_busy[3] && `SPC6.spu.mct.spu_pmu_ma_busy[2:0] == 6)))
//         @(negedge `SPC6.l2clk);
...
//     end
    ...
    begin : FORK9_6
      // start step. tid wakes up/redirects and does "one instruction"
      while (!(`SPC6.tlu_trap_pc_1_valid && `SPC6.tlu_trap_1_tid[1:0] == 2 && `SPC6.tlu.tlu_retry[1] == 1)) begin
        // might see  `SPC6.tlu.mmu_reload_done[6] here if previous
        // redirect was due to a MMU miss. If so, decrement flush count because
        // the previous flush on prev pc_valid "does not count" and there can be
        // any number of them w/ no way to predict quantity.
        if (`SPC6.tlu.mmu_reload_done[6] && ssMode) begin
      ...
      @(negedge `SPC6.pku_quiesce[6]);    
      `PR_DEBUG("dbg_chk", `DEBUG, "watchStep6: TID 6 got negedge pku_quiesce, step waiting on dec_inst_valid_m!");
    
      while (!(`SPC6.dec_inst_valid_m[1] == 1 && `SPC6.dec_tid1_m[1:0] == 2)) @(negedge `SPC6.l2clk);
      ...
      //       if (!`SPC6.spu_tlu_cwq_busy && 
//           !(`SPC6.spu.mct.spu_pmu_ma_busy[3] && 
//             `SPC6.spu.mct.spu_pmu_ma_busy[2:0] == 6)) spuActive[6] = 0;
    end
  join
...
//     begin : FORK12_6
//       while (!(`SPC6.spu_tlu_cwq_busy || 
//         ((`SPC6.spu.mct.spu_pmu_ma_busy[3] && `SPC6.spu.mct.spu_pmu_ma_busy[2:0] == 6 && doMode) || (`SPC6.spu.mct.spu_pmu_ma_busy[3] && ssMode))))
//         @(negedge `SPC6.l2clk);
//       spuActive[6] = 1;
//     end
    ...
    begin : FORK6_6
        // checking ptrap_flush to make sure that flush is not caused by lsu_trap_flush
        while (`SPC6.tlu_flush_ifu[6] == 0)
          @(negedge `SPC6.l2clk);

// // debug!!!!!!!!!!!!!!!!!!!!!!!!!!!
// if (`SPC6.tlu.fls1.ptrap_flush[2] == 1) begin
//   `PR_ERROR ("dbg_chk", `ERROR, "watchStep6: TID 6 have tlu_flush_ifu !!!!!!!!!!!!!!!!!!!!!!!");
// end
// 

        if ((ssMode && `SPC6.tlu.fls1.ptrap_flush[2] == 0) || doMode) begin
    ...
    begin : FORK5_6
      if (doMode) begin
        ...
        while (`SPC6.pku_quiesce[6] == 0 ||
               `SPC6.ftu_ifu_quiesce[6] == 0 ||
               `SPC6.lsu_stb_empty[6] == 0) @(negedge `SPC6.l2clk);
...
//         while (ssMode && !`SPC6.spc_ss_complete) begin
//           // detect a second inst valid w/o a flush between
//           if (`SPC6.tlu_trap_pc_1_valid && `SPC6.tlu_trap_1_tid[1:0] == 2 && `SPC6.tlu.tlu_retry[1] == 1) begin
//             // delay error so flush thread can kill this in the "got flush" case
//             @(negedge `SPC6.l2clk); 
...
begin
  ...
  if (! `TOP.cpu.tcu_do_mode[6]) disable watchDOsteps6;

  // now wait for a thread to start. TCU internal signal.
  if (`SPC6.tlu.tlu_core_running_status[6] == 0)
    @(posedge `SPC6.tlu.tlu_core_running_status[6]);

  `PR_DEBUG("dbg_chk", `DEBUG, "watchDOsteps6 TID 6 unparked and running in DO mode!");

  // loop on step checking. DO mode is self stepping
  while (doMode) begin : watchDOsteps6loop// && `SPC6.tlu.tlu_core_running_status[7:0] != 0) begin
...
// got park request. transition into parking.  parking check.
always @(negedge `SPC6.tlu.tcu_core_running[7]) begin: GOTPARK7
  `PR_DEBUG("dbg_chk", `DEBUG, "TID 7 calling parkingCheck");
  if (enabled) parkingCheck7();
end

// got unpark request. transition into running. unparking check.
always @(posedge `SPC6.tlu.tcu_core_running[7]) begin: UNPARKCHECK7
  if (enabled && !ssMode) begin
...
// check for unexpected running transition
  always @(`SPC6.tlu.tlu_core_running_status[7]) begin: UNEXPECTED7
...
begin
  ...
  // need to see at least one assertion of flush
  // if (`SPC6.tlu_flush_ifu[7] !== 1) 
  //   @(posedge `SPC6.tlu_flush_ifu[7]); // has glitches
  if (`SPC6.ifu_ftu.ftu_agc_ctl.tlu_flush_ifu_f[7] !== 1)
    @(posedge `SPC6.ifu_ftu.ftu_agc_ctl.tlu_flush_ifu_f[7]);

  // need to see all of these go idle
  while (`SPC6.pku_quiesce[7] == 0 ||
         `SPC6.ftu_ifu_quiesce[7] == 0 ||
         `SPC6.lsu_stb_empty[7] == 0) 
     @(negedge `SPC6.l2clk);
...
begin //{
  ...
  while ((parkTrans[7] == 0 && ssMode == 0) || 
         (virtualPark[7] == 1 && ssMode == 1 && 
          `TOP.cpu.tcu_ss_request[6] == 0)) begin //{
    `PR_DEBUG ("dbg_chk", `DEBUG, "TID 7 watching for activity while parked/between steps. trans=%0h, virtualPark=%0h, tcu_ss_request=%0h",parkTrans[7],virtualPark[7],`TOP.cpu.tcu_ss_request[6]);
    ...
    // (core_running_status=0). This looks like an un-asked for step so it
    // has to be detected and ignored. Try stalling here during it.
    while (`SPC6.tlu.tcu_core_running[7] == 0 && 
           `SPC6.tlu.tlu_core_running_status[7] == 1) 
      @(negedge `SPC6.l2clk);

    // watch for instructions sneaking in
    // dec_inst_valid_m[1:0], one bit per group
    if (`SPC6.tlu_trap_pc_1_valid && `SPC6.tlu_trap_1_tid[1:0] == 7 && `SPC6.tlu.tlu_retry[1] == 1)
      `PR_ERROR ("dbg_chk", `ERROR, "ERROR: TID 7 had tlu_trap_pc_1_valid activity while parked/between steps!");

    if (`SPC6.dec_inst_valid_m[1] == 1 && `SPC6.dec_tid1_m[1:0] == 3)
      `PR_ERROR ("dbg_chk", `ERROR, "ERROR: TID 7 had dec_inst_valid_m activity while parked/between steps!");


  end //} while

  `PR_DEBUG ("dbg_chk", `DEBUG, "TID 7 STOP watching for activity while parked/between steps. trans=%0h, virtualPark=%0h, tcu_ss_request=%0h",parkTrans[7],virtualPark[7],`TOP.cpu.tcu_ss_request[6]);
...
begin
  ...
  // must see `SPC6.tlu.core_running_status[7] w/in count clocks
  count7 = 0;

  while (`SPC6.tlu.tlu_core_running_status[7] == 0) begin
    @(negedge `SPC6.l2clk);
  ...
  if (!doMode && !ssMode) begin
    ...
    // happened at once so while loop can be skipped.
      while (! (`SPC6.tlu_trap_pc_1_valid && 
                `SPC6.tlu_trap_1_tid[1:0] == 3)) begin
        @(negedge `SPC6.l2clk);
...
//     // look for SPU going active    
//     begin : FORK11_7
//       while (!(`SPC6.spu_tlu_cwq_busy || (`SPC6.spu.mct.spu_pmu_ma_busy[3] && `SPC6.spu.mct.spu_pmu_ma_busy[2:0] == 7)))
//         @(negedge `SPC6.l2clk);
...
//     end
    ...
    begin : FORK9_7
      // start step. tid wakes up/redirects and does "one instruction"
      while (!(`SPC6.tlu_trap_pc_1_valid && `SPC6.tlu_trap_1_tid[1:0] == 3 && `SPC6.tlu.tlu_retry[1] == 1)) begin
        // might see  `SPC6.tlu.mmu_reload_done[7] here if previous
        // redirect was due to a MMU miss. If so, decrement flush count because
        // the previous flush on prev pc_valid "does not count" and there can be
        // any number of them w/ no way to predict quantity.
        if (`SPC6.tlu.mmu_reload_done[7] && ssMode) begin
      ...
      @(negedge `SPC6.pku_quiesce[7]);    
      `PR_DEBUG("dbg_chk", `DEBUG, "watchStep7: TID 7 got negedge pku_quiesce, step waiting on dec_inst_valid_m!");
    
      while (!(`SPC6.dec_inst_valid_m[1] == 1 && `SPC6.dec_tid1_m[1:0] == 3)) @(negedge `SPC6.l2clk);
      ...
      //       if (!`SPC6.spu_tlu_cwq_busy && 
//           !(`SPC6.spu.mct.spu_pmu_ma_busy[3] && 
//             `SPC6.spu.mct.spu_pmu_ma_busy[2:0] == 7)) spuActive[7] = 0;
    end
  join
...
//     begin : FORK12_7
//       while (!(`SPC6.spu_tlu_cwq_busy || 
//         ((`SPC6.spu.mct.spu_pmu_ma_busy[3] && `SPC6.spu.mct.spu_pmu_ma_busy[2:0] == 7 && doMode) || (`SPC6.spu.mct.spu_pmu_ma_busy[3] && ssMode))))
//         @(negedge `SPC6.l2clk);
//       spuActive[7] = 1;
//     end
    ...
    begin : FORK6_7
        // checking ptrap_flush to make sure that flush is not caused by lsu_trap_flush
        while (`SPC6.tlu_flush_ifu[7] == 0)
          @(negedge `SPC6.l2clk);

// // debug!!!!!!!!!!!!!!!!!!!!!!!!!!!
// if (`SPC6.tlu.fls1.ptrap_flush[3] == 1) begin
//   `PR_ERROR ("dbg_chk", `ERROR, "watchStep7: TID 7 have tlu_flush_ifu !!!!!!!!!!!!!!!!!!!!!!!");
// end
// 

        if ((ssMode && `SPC6.tlu.fls1.ptrap_flush[3] == 0) || doMode) begin
    ...
    begin : FORK5_7
      if (doMode) begin
        ...
        while (`SPC6.pku_quiesce[7] == 0 ||
               `SPC6.ftu_ifu_quiesce[7] == 0 ||
               `SPC6.lsu_stb_empty[7] == 0) @(negedge `SPC6.l2clk);
...
//         while (ssMode && !`SPC6.spc_ss_complete) begin
//           // detect a second inst valid w/o a flush between
//           if (`SPC6.tlu_trap_pc_1_valid && `SPC6.tlu_trap_1_tid[1:0] == 3 && `SPC6.tlu.tlu_retry[1] == 1) begin
//             // delay error so flush thread can kill this in the "got flush" case
//             @(negedge `SPC6.l2clk); 
...
begin
  ...
  if (! `TOP.cpu.tcu_do_mode[6]) disable watchDOsteps7;

  // now wait for a thread to start. TCU internal signal.
  if (`SPC6.tlu.tlu_core_running_status[7] == 0)
    @(posedge `SPC6.tlu.tlu_core_running_status[7]);

  `PR_DEBUG("dbg_chk", `DEBUG, "watchDOsteps7 TID 7 unparked and running in DO mode!");

  // loop on step checking. DO mode is self stepping
  while (doMode) begin : watchDOsteps7loop// && `SPC6.tlu.tlu_core_running_status[7:0] != 0) begin
...
always @(posedge `TOP.cpu.tcu_do_mode[6]) begin: DOMODE
  if (enabled) begin
    ...
    if (`SPC6.tlu.tcu_core_running[7:0] == 0) 
      @(`SPC6.tlu.tcu_core_running[7:0]) 

    stepAllowed[7:0] = `SPC6.tlu.tcu_core_running[7:0];
...
always @(negedge `TOP.cpu.tcu_do_mode[6]) begin
  if (enabled) begin
...
always @(posedge `TOP.cpu.tcu_ss_mode[6]) begin: SSMODE

  if (enabled) begin
 
    // initial value in case ss_mode and tcu_core_running change at once
    stepAllowed[7:0] = `SPC6.tlu.tcu_core_running[7:0]; // need?
    ...
    if (`SPC6.tlu.tcu_core_running[7:0] == 0) 
      @(`SPC6.tlu.tcu_core_running[7:0]) stepAllowed[7:0] = `SPC6.tlu.tcu_core_running[7:0];

    `PR_DEBUG("dbg_chk", `DEBUG, "SS mode, ready to run in SS mode!");

    // loop on this step checking
    while (`TOP.cpu.tcu_ss_mode[6] === 1) begin
  
      // wait for TCU to request a step or for SS mode to end
      @(posedge `TOP.cpu.tcu_ss_request or negedge `TOP.cpu.tcu_ss_mode[6]);

      if (`TOP.cpu.tcu_ss_mode[6] === 1) begin
...
//                   `SPC6.spu.mct.spu_pmu_ma_busy[2:0] == 7)) spuActive[7] = 0;
        ...
        if (stepDone[7:0] !== `SPC6.tlu.tcu_core_running[7:0])
           `PR_ERROR ("dbg_chk", `ERROR, "ERROR: SS mode stepDone not correct (%b/%b/%b stepAllowed,stepDone,core_running)!",stepAllowed, stepDone, `SPC6.tlu.tcu_core_running[7:0]);
...
//         end
//         @(posedge `SPC6.spc_ss_complete) disable FORK2;
//       join

      // DUT must have asserted ss_complete
//       if (!`SPC6.spc_ss_complete)
//         `PR_ERROR ("dbg_chk", `ERROR, "ERROR: Not seeing ss_complete after all threads stepped. Checker problem?");
        ...
        if (virtualPark[7:0] !== `SPC6.tlu.tcu_core_running[7:0])
...
always @(negedge `TOP.cpu.tcu_ss_mode[6]) begin
  if (enabled) begin
    `PR_DEBUG("dbg_chk", `DEBUG, "Clearing out Single Step mode state!");
    ssMode = 0;
    stepAllowed = 0;
    virtualPark = 0;
    if (!(`SPC6.tcu_core_running[7:0] == 0 && 
        `SPC6.spc_core_running_status[7:0] == 8'h0))
...
// 
always @(posedge `TOP.cpu.tcu_ss_mode[6] or posedge `TOP.cpu.tcu_do_mode[6]) begin
  if (enabled) begin
    while (`TOP.cpu.tcu_ss_mode[6] || `TOP.cpu.tcu_do_mode[6]) begin
      @(negedge `SPC6.l2clk);
      if (`TOP.cpu.spc6.ifu_ibu.ibq0.buf_valid_p[7:1] != 0 && 
          `TOP.cpu.spc6.ifu_ibu.ibq0.instr_sf_valid_except_p == 0 &&
          `SPC6.tcu_core_running[0])
        `PR_ERROR ("dbg_chk", `ERROR, "TID 0 had ifetch buffer(s) valid other than zero (buf_valid_p[7:1]=%0b)!",`TOP.cpu.spc6.ifu_ibu.ibq0.buf_valid_p[7:1]);
    end      
  end
end
...
always @(posedge `TOP.cpu.tcu_ss_mode[6] or posedge `TOP.cpu.tcu_do_mode[6]) begin
  if (enabled) begin
    while (`TOP.cpu.tcu_ss_mode[6] || `TOP.cpu.tcu_do_mode[6]) begin
      @(negedge `SPC6.l2clk);
      if (`TOP.cpu.spc6.ifu_ibu.ibq1.buf_valid_p[7:1] != 0 && 
          `TOP.cpu.spc6.ifu_ibu.ibq1.instr_sf_valid_except_p == 0 &&
          `SPC6.tcu_core_running[1])
        `PR_ERROR ("dbg_chk", `ERROR, "TID 1 had ifetch buffer(s) valid other than zero (buf_valid_p[7:1]=%0b)!",`TOP.cpu.spc6.ifu_ibu.ibq1.buf_valid_p[7:1]);
    end      
  end
end
...
always @(posedge `TOP.cpu.tcu_ss_mode[6] or posedge `TOP.cpu.tcu_do_mode[6]) begin
  if (enabled) begin
    while (`TOP.cpu.tcu_ss_mode[6] || `TOP.cpu.tcu_do_mode[6]) begin
      @(negedge `SPC6.l2clk);
      if (`TOP.cpu.spc6.ifu_ibu.ibq2.buf_valid_p[7:1] != 0 && 
          `TOP.cpu.spc6.ifu_ibu.ibq2.instr_sf_valid_except_p == 0 &&
          `SPC6.tcu_core_running[2])
        `PR_ERROR ("dbg_chk", `ERROR, "TID 2 had ifetch buffer(s) valid other than zero (buf_valid_p[7:1]=%0b)!",`TOP.cpu.spc6.ifu_ibu.ibq2.buf_valid_p[7:1]);
    end      
  end
end
...
always @(posedge `TOP.cpu.tcu_ss_mode[6] or posedge `TOP.cpu.tcu_do_mode[6]) begin
  if (enabled) begin
    while (`TOP.cpu.tcu_ss_mode[6] || `TOP.cpu.tcu_do_mode[6]) begin
      @(negedge `SPC6.l2clk);
      if (`TOP.cpu.spc6.ifu_ibu.ibq3.buf_valid_p[7:1] != 0 && 
          `TOP.cpu.spc6.ifu_ibu.ibq3.instr_sf_valid_except_p == 0 &&
          `SPC6.tcu_core_running[3])
        `PR_ERROR ("dbg_chk", `ERROR, "TID 3 had ifetch buffer(s) valid other than zero (buf_valid_p[7:1]=%0b)!",`TOP.cpu.spc6.ifu_ibu.ibq3.buf_valid_p[7:1]);
    end      
  end
end
...
always @(posedge `TOP.cpu.tcu_ss_mode[6] or posedge `TOP.cpu.tcu_do_mode[6]) begin
  if (enabled) begin
    while (`TOP.cpu.tcu_ss_mode[6] || `TOP.cpu.tcu_do_mode[6]) begin
      @(negedge `SPC6.l2clk);
      if (`TOP.cpu.spc6.ifu_ibu.ibq4.buf_valid_p[7:1] != 0 && 
          `TOP.cpu.spc6.ifu_ibu.ibq4.instr_sf_valid_except_p == 0 &&
          `SPC6.tcu_core_running[4])
        `PR_ERROR ("dbg_chk", `ERROR, "TID 4 had ifetch buffer(s) valid other than zero (buf_valid_p[7:1]=%0b)!",`TOP.cpu.spc6.ifu_ibu.ibq4.buf_valid_p[7:1]);
    end      
  end
end
...
always @(posedge `TOP.cpu.tcu_ss_mode[6] or posedge `TOP.cpu.tcu_do_mode[6]) begin
  if (enabled) begin
    while (`TOP.cpu.tcu_ss_mode[6] || `TOP.cpu.tcu_do_mode[6]) begin
      @(negedge `SPC6.l2clk);
      if (`TOP.cpu.spc6.ifu_ibu.ibq5.buf_valid_p[7:1] != 0 && 
          `TOP.cpu.spc6.ifu_ibu.ibq5.instr_sf_valid_except_p == 0 &&
          `SPC6.tcu_core_running[5])
        `PR_ERROR ("dbg_chk", `ERROR, "TID 5 had ifetch buffer(s) valid other than zero (buf_valid_p[7:1]=%0b)!",`TOP.cpu.spc6.ifu_ibu.ibq5.buf_valid_p[7:1]);
    end      
  end
end
...
always @(posedge `TOP.cpu.tcu_ss_mode[6] or posedge `TOP.cpu.tcu_do_mode[6]) begin
  if (enabled) begin
    while (`TOP.cpu.tcu_ss_mode[6] || `TOP.cpu.tcu_do_mode[6]) begin
      @(negedge `SPC6.l2clk);
      if (`TOP.cpu.spc6.ifu_ibu.ibq6.buf_valid_p[7:1] != 0 && 
          `TOP.cpu.spc6.ifu_ibu.ibq6.instr_sf_valid_except_p == 0 &&
          `SPC6.tcu_core_running[6])
        `PR_ERROR ("dbg_chk", `ERROR, "TID 6 had ifetch buffer(s) valid other than zero (buf_valid_p[7:1]=%0b)!",`TOP.cpu.spc6.ifu_ibu.ibq6.buf_valid_p[7:1]);
    end      
  end
end
...
always @(posedge `TOP.cpu.tcu_ss_mode[6] or posedge `TOP.cpu.tcu_do_mode[6]) begin
  if (enabled) begin
    while (`TOP.cpu.tcu_ss_mode[6] || `TOP.cpu.tcu_do_mode[6]) begin
      @(negedge `SPC6.l2clk);
      if (`TOP.cpu.spc6.ifu_ibu.ibq7.buf_valid_p[7:1] != 0 && 
          `TOP.cpu.spc6.ifu_ibu.ibq7.instr_sf_valid_except_p == 0 &&
          `SPC6.tcu_core_running[7])
        `PR_ERROR ("dbg_chk", `ERROR, "TID 7 had ifetch buffer(s) valid other than zero (buf_valid_p[7:1]=%0b)!",`TOP.cpu.spc6.ifu_ibu.ibq7.buf_valid_p[7:1]);
    end      
  end
end
...
always @(posedge `TOP.cpu.tcu_ss_mode[6] or posedge `TOP.cpu.tcu_do_mode[6]) begin
  if (enabled) begin
    ...
    while (`TOP.cpu.tcu_ss_mode[6] || `TOP.cpu.tcu_do_mode[6]) begin
      @(negedge `SPC6.l2clk);
      if (`TOP.cpu.spc6.tlu.dec_tid0_m[1:0] == 0 && `TOP.cpu.spc6.tlu.dec_inst_valid_m[0] == 1 && `SPC6.tcu_core_running[0] && (`TOP.cpu.tcu_ss_mode[6] || `TOP.cpu.tcu_do_mode[6])) begin
        // have inst
        if (pipelineCount0 < 9) begin
          `PR_ERROR ("dbg_chk", `ERROR, "TID 0 had pipelining in DO or SS mode (pipelineCount=%0d)!",pipelineCount0);
          //stall
          @(negedge `TOP.cpu.tcu_ss_mode[6] or negedge `TOP.cpu.tcu_do_mode[6]);
        end
...
always @(posedge `TOP.cpu.tcu_ss_mode[6] or posedge `TOP.cpu.tcu_do_mode[6]) begin
  if (enabled) begin
    ...
    while (`TOP.cpu.tcu_ss_mode[6] || `TOP.cpu.tcu_do_mode[6]) begin
      @(negedge `SPC6.l2clk);
      if (`TOP.cpu.spc6.tlu.dec_tid0_m[1:0] == 1 && `TOP.cpu.spc6.tlu.dec_inst_valid_m[0] == 1 && `SPC6.tcu_core_running[1] && (`TOP.cpu.tcu_ss_mode[6] || `TOP.cpu.tcu_do_mode[6])) begin
        // have inst
...
always @(posedge `TOP.cpu.tcu_ss_mode[6] or posedge `TOP.cpu.tcu_do_mode[6]) begin
  if (enabled) begin
    ...
    while (`TOP.cpu.tcu_ss_mode[6] || `TOP.cpu.tcu_do_mode[6]) begin
      @(negedge `SPC6.l2clk);
      if (`TOP.cpu.spc6.tlu.dec_tid0_m[1:0] == 2 && `TOP.cpu.spc6.tlu.dec_inst_valid_m[0] == 1 && `SPC6.tcu_core_running[2] && (`TOP.cpu.tcu_ss_mode[6] || `TOP.cpu.tcu_do_mode[6])) begin
        // have inst
...
always @(posedge `TOP.cpu.tcu_ss_mode[6] or posedge `TOP.cpu.tcu_do_mode[6]) begin
  if (enabled) begin
    ...
    while (`TOP.cpu.tcu_ss_mode[6] || `TOP.cpu.tcu_do_mode[6]) begin
      @(negedge `SPC6.l2clk);
      if (`TOP.cpu.spc6.tlu.dec_tid0_m[1:0] == 3 && `TOP.cpu.spc6.tlu.dec_inst_valid_m[0] == 1 && `SPC6.tcu_core_running[3] && (`TOP.cpu.tcu_ss_mode[6] || `TOP.cpu.tcu_do_mode[6])) begin
        // have inst
...
always @(posedge `TOP.cpu.tcu_ss_mode[6] or posedge `TOP.cpu.tcu_do_mode[6]) begin
  if (enabled) begin
    ...
    while (`TOP.cpu.tcu_ss_mode[6] || `TOP.cpu.tcu_do_mode[6]) begin
      @(negedge `SPC6.l2clk);
      if (`TOP.cpu.spc6.tlu.dec_tid1_m[1:0] == 0 && `TOP.cpu.spc6.tlu.dec_inst_valid_m[1] == 1 && `SPC6.tcu_core_running[4] && (`TOP.cpu.tcu_ss_mode[6] || `TOP.cpu.tcu_do_mode[6])) begin
        // have inst
...
always @(posedge `TOP.cpu.tcu_ss_mode[6] or posedge `TOP.cpu.tcu_do_mode[6]) begin
  if (enabled) begin
    ...
    while (`TOP.cpu.tcu_ss_mode[6] || `TOP.cpu.tcu_do_mode[6]) begin
      @(negedge `SPC6.l2clk);
      if (`TOP.cpu.spc6.tlu.dec_tid1_m[1:0] == 1 && `TOP.cpu.spc6.tlu.dec_inst_valid_m[1] == 1 && `SPC6.tcu_core_running[5] && (`TOP.cpu.tcu_ss_mode[6] || `TOP.cpu.tcu_do_mode[6])) begin
        // have inst
...
always @(posedge `TOP.cpu.tcu_ss_mode[6] or posedge `TOP.cpu.tcu_do_mode[6]) begin
  if (enabled) begin
    ...
    while (`TOP.cpu.tcu_ss_mode[6] || `TOP.cpu.tcu_do_mode[6]) begin
      @(negedge `SPC6.l2clk);
      if (`TOP.cpu.spc6.tlu.dec_tid1_m[1:0] == 2 && `TOP.cpu.spc6.tlu.dec_inst_valid_m[1] == 1 && `SPC6.tcu_core_running[6] && (`TOP.cpu.tcu_ss_mode[6] || `TOP.cpu.tcu_do_mode[6])) begin
        // have inst
...
always @(posedge `TOP.cpu.tcu_ss_mode[6] or posedge `TOP.cpu.tcu_do_mode[6]) begin
  if (enabled) begin
    ...
    while (`TOP.cpu.tcu_ss_mode[6] || `TOP.cpu.tcu_do_mode[6]) begin
      @(negedge `SPC6.l2clk);
      if (`TOP.cpu.spc6.tlu.dec_tid1_m[1:0] == 3 && `TOP.cpu.spc6.tlu.dec_inst_valid_m[1] == 1 && `SPC6.tcu_core_running[7] && (`TOP.cpu.tcu_ss_mode[6] || `TOP.cpu.tcu_do_mode[6])) begin
        // have inst
...
always @(negedge `SPC6.l2clk) begin
  ...
  if (!noCmtCheck && enabled) begin
    ...
    if (cmtValid0 !== `TOP.cpu.spc6.tlu_dbg_instr_cmt_grp0[1:0]) begin
      `PR_ERROR ("dbg_chk", `ERROR, "tlu_dbg_instr_cmt_grp0 not right, (cmtValid0=%0h, tlu_dbg_instr_cmt_grp0=%0h)",cmtValid0,`TOP.cpu.spc6.tlu_dbg_instr_cmt_grp0[1:0]);
    ...
    if (`TOP.cpu.spc6.tlu.fls0.inst_valid_w && 
        !(`TOP.cpu.spc6.tlu.fls0.flush_ifu[3:0] & 
          `TOP.cpu.spc6.tlu.fls0.tid_dec_w[3:0])) begin
       if (`TOP.cpu.spc6.tlu.fls0.cti_w) cmtValid0 = 1;
       else if (`TOP.cpu.spc6.tlu.fls0.lsu_inst_w) cmtValid0 = 3;
       else cmtValid0 = 2;
    end
    else cmtValid0 = 0;
  end
...
always @(negedge `SPC6.l2clk) begin
  ...
  if (!noCmtCheck && enabled) begin
    ...
    // should we see non zero this cycle?
    if (cmtValid1 !== `TOP.cpu.spc6.tlu_dbg_instr_cmt_grp1[1:0]) begin
      `PR_ERROR ("dbg_chk", `ERROR, "tlu_dbg_instr_cmt_grp1 not right, (cmtValid1=%0h, tlu_dbg_instr_cmt_grp1=%0h)",cmtValid1,`TOP.cpu.spc6.tlu_dbg_instr_cmt_grp1[1:0]);
      `PR_ALWAYS ("dbg_chk", `ALWAYS, "tlu_dbg_instr_cmt_grp1 not right, IF THE VALUE IS X, SPC IS NOT RESET OR SOME SPC INPUT IS X!");
    end

    // should see non zero next cycle
    if (`TOP.cpu.spc6.tlu.fls1.inst_valid_w && 
        !(`TOP.cpu.spc6.tlu.fls1.flush_ifu[3:0] & 
          `TOP.cpu.spc6.tlu.fls1.tid_dec_w[3:0])) begin
       if (`TOP.cpu.spc6.tlu.fls1.cti_w) cmtValid1 = 1;
       else if (`TOP.cpu.spc6.tlu.fls1.lsu_inst_w) cmtValid1 = 3;
       else cmtValid1 = 2;
    end
    else cmtValid1 = 0;
  end
...
// second count tid 0
always @(negedge `SPC6.l2clk) begin
  if(`TOP.nas_top.c6.t0.complete_fw2[7:0]) cmtCountT0 = cmtCountT0+1;
  if(`TOP.nas_top.c6.t1.complete_fw2[7:0]) cmtCountT1 = cmtCountT1+1;
  if(`TOP.nas_top.c6.t2.complete_fw2[7:0]) cmtCountT2 = cmtCountT2+1;
  if(`TOP.nas_top.c6.t3.complete_fw2[7:0]) cmtCountT3 = cmtCountT3+1;
end
