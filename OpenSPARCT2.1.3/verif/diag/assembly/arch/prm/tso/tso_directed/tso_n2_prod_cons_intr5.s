...
* 
* OpenSPARC T2 Processor File: tso_n2_prod_cons_intr5.s
...
main:  
       ...
       setx prodCntBase, %g5, %g6
       mov  0x04, %g7
       mov  NO_OF_STRANDS, %l0
prodCntInit:
       st    %g7, [%g6]
       subcc %l0, 1, %l0
       bnz   prodCntInit
...
consCntInit:
       ...
       setx consumedDataBase, %g5, %g6
       mov  0x0, %g7
       mov  NO_OF_STRANDS, %l0
consumedDataInit:
       st    %g7, [%g6]
       subcc %l0, 1, %l0
       bnz   consumedDataInit
       nop 

       setx producedDataBase, %g5, %g6
       mov  0x0, %g7
       mov  NO_OF_STRANDS, %l0
producedDataInit:
       st    %g7, [%g6]
       subcc %l0, 1, %l0
       bnz   producedDataInit
       nop 
...
nop
wakeProducer:
setx prodCntBase , %l0, %l2
umul thrdIdReg, 0x4, %g4 
lduw [%l2 + %g4], %l3
subcc %l3, 0x1, %l3
subcc %l3, %g0, %g0
...
!dispacth interrrupt to producer
   ...
   sub  %g7, 1, %g7
   sllx %g7, 0x7, %g7
   or	%g7,0x001, %g7			! send to thread 0 vector number 1
   stxa	%g7, [%g0]ASI_SWVR_UDB_INTR_W   ! no membar sync needed after this reg

   ba    snoozeConsumer
   nop
consumeComplete:
   sub	  thrdIdReg, 1, %g7			! send to thread 0 vector number 1
   sllx   %g7, 0x7, %g6
   or      %g6, 0x2, %g7
   stxa	%g7, [%g0]ASI_SWVR_UDB_INTR_W   ! no membar sync needed after this reg
   ta    T_GOOD_TRAP
