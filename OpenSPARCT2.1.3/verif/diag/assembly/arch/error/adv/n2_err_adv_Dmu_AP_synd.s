...
* 
* OpenSPARC T2 Processor File: n2_err_adv_Dmu_AP_synd.s
...
#define DMA_DATA_BYP_ADDR2      0xfffc0001234e6700
#define DMA_DATA_BYP_ADDR3      0xfffc0001234f6700
...
eie_reg_ones_rdd:
	...
	set	0x1, %g1			! 1 Trap; only for NCU DmuDParity
...
eie_reg_ones_wri:
        ...
        set     0x2, %g1                       	! 1 traps from rdd; 1 trap from WRI 
        setx    0x100, %g7, %g6
err_trap_loop_wri:
        cmp     %g6, %g0
        be      %xcc, test_failed
...
eie_reg_ones_wrm:
        ...
        set     0x4, %g1                       	! 2 traps already from rdd and wri 
						! 2 Traps ; one for NCU DmuDParity
                                                ! another for L2 DRU
        setx    0x100, %g7, %g6
err_trap_loop_wrm:
        cmp     %g6, %g0
...
cmp_err_cnt_tt40:
	mov	0x1, %g1
	cmp	%i7, %g1
	be	synd_rdd_err_tt40
	nop

        mov     0x2, %g1
        cmp     %i7, %g1
        be      synd_wri_err_tt40
        nop

        mov     0x3, %g1
        cmp     %i7, %g1
        be      synd_wrm_err_tt40
        nop
...
synd_rdd_err_tt40:
	setx	SOC_SII_ERR_SYND_REG, %g7, %g5
	ldx	[%g5], %g1
	setx	0x8700010123456500, %g7, %g2
	cmp	%g1, %g2
	bne	%xcc, test_failed 
	nop
...
synd_wri_err_tt40:
	setx	SOC_SII_ERR_SYND_REG, %g7, %g5
	ldx	[%g5], %g1
	setx	0xff0000ffffffffff, %g7, %g6
	and	%g1, %g6, %g4
	setx	0x87000001234e6500, %g7, %g2
        cmp     %g4, %g2
        bne     %xcc, test_failed 
        nop
