...
* 
* OpenSPARC T2 Processor File: n2_err_dmu_dma_rd.s
...
! see if this user event works in a loop (multi-shot)
	...
	setx	0x02, %g1, %g4
XmtUsrEvnt1:
	nop         ! $EV trig_pc_d(1, @VA(.MAIN.XmtUsrEvnt1)) -> EnablePCIeIgCmd ("DMARD", DMA_DATA_BYP_SADDR, DMA_DATA_BYP_EADDR, "64'h40", 1, *, * )
	dec	%g4
	brnz	%g4, XmtUsrEvnt1
	nop
	nop
...
XmtUsrEvnt2:
	! $EV trig_pc_d(1, @VA(.MAIN.XmtUsrEvnt2)) -> EnablePCIeIgCmd ("DMARD", DMA_DATA_BYP_SADDR, DMA_DATA_BYP_EADDR, "64'h100", 1, *, * )
	
	! select a MEM32 address in PCI address range and transmit the command to NCU
	
	setx    MEM32_RD_ADDR, %g1, %g2
	setx	0x04, %g1, %g4		! loop 4 times
...
delay_loop:
	...
	add	%g2, 8, %g2		! increment PIO  address by 8
