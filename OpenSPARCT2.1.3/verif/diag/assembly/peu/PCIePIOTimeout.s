...
* 
* OpenSPARC T2 Processor File: PCIePIOTimeout.s
 ...
 ******************************************************************************/
	...
	! select a CFG1 address in PCI address range and transmit the command to NCU
...
/******************************************************************************
 On the next PIO, get a timeout error (no completion)
 ******************************************************************************/
! $EV trig_pc_d(1, @VA(.MAIN.PIO_Timeout)) -> EnablePCIeIgCmd ("PIO_TIMEOUT",0,0,0,1)
PIO_Timeout: nop; nop; nop; nop; nop; nop; nop; nop;
	lduw [%g2 + 5*8], %o4	! this one should timeout
	
	lduw [%g2 + 6*8], %o5	! this one should be good
	lduw [%g2 + 7*8], %o6	! this one should be good
	
	ldx  [%g3], %g4		! OE status => should get the CTO bit set
	setx 0x0000000000200000, %g1, %l1
	xor  %l1, %g4, %l1
	brnz %l1, test_failed
	stx  %g4, [%g3]		! clear it
	...
	ldx  [%g5], %g6		! UE status => should get the CTO bit set
	setx 0x0000000000004000, %g1, %l1
	xor  %l1, %g6, %l1
	brnz %l1, test_failed
	stx  %g6, [%g5]		! clear it
