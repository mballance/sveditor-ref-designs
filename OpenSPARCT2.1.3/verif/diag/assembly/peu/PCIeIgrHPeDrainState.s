...
* 
* OpenSPARC T2 Processor File: PCIeIgrHPeDrainState.s
...
! here is where we force an error
...
! $EV trig_pc_d(1,@VA(.MAIN.Check_drain_state)) -> EnablePCIeIgCmd("PIO_NOEXP",0,0,0,1)
	...
	best_set_reg( 0x00000f20, %g1, %l4 )
Setup_IHB_Err_Inject:
	nop
        stx	%l4, [%g4]	! set the IHP error injection triggers
	mov	0, %l4
	
	ldx	[%g2 + 16], %l0		! PIO read ===> should get the IHP error
	
	dec	%l4			! an interrupt should have occured
	brnz    %l4, test_failed
	

! now wait for the "Igress hdr buffer Parity Error" primary or secondary status to be set
	setx	FIRE_DLC_ILU_CIB_CSR_A_ILU_LOG_ERR_RW1C_ALIAS_ADDR, %g1, %g3
	mov	10, %l0
        setx	0x0000001000000010, %g1, %l1	! mask for IHP Primary and Secondary Events
...
Wait_for_IHP:
	...
	nop
	dec	%l0
	brnz	%l0, Wait_for_IHP
	nop
	b       test_failed
	nop
	
/****************************************************************
 check that drain.state bit is NOT set in the PEU Status Register
 ****************************************************************/
Check_drain_state:
	mov	0, %g1
	stx	%g1, [%g4]	! clear the EDP error injection triggers
...
! $EV trig_pc_d(1,@VA(.MAIN.Check_drain_state)) -> EnablePCIeIgCmd("PIO_NOEXP",0,0,0,1)
	...
	mov	0x100, %g5
	andcc   %g4, %g5, %l0
	bnz	test_failed		! branch if drain state IS set
	nop	
...
Wait_for_dma_read_clear:
	...
	! clear the IHP err bit
	setx	FIRE_DLC_ILU_CIB_CSR_A_ILU_LOG_ERR_RW1C_ALIAS_ADDR, %g1, %g3
        ldx	[%g3], %l3
	stx	%l3, [%g3]		! RW1C
        ldx	[%g3], %l1
	brnz	%l1, test_failed	! branch if IHP err is not cleared
	nop
