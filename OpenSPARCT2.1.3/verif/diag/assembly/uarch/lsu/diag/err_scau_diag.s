...
* 
* OpenSPARC T2 Processor File: err_scau_diag.s
...
main:		/* test begin */
  ...
  !! set CERER.SCAU
  setx 0x20000000, %g1, %o2   !! enable scac errs
...
!! With bit 0 = 1: 70, 60, 50, 40, 30, 20, 10  => 7 cases
!! With bit 1 = 1: 71, 61, 51, 41, 31, 21 => 6 cases
!! With bit 2 = 1: 72, 62, 52, 42, 32 => 5 cases
!! With bit 3 = 1: 73, 63, 53, 43 => 4 cases
!! With bit 4 = 1: 74, 64, 54 => 3 cases
!! With bit 5 = 1: 75, 65 => 2 cases
!! With bit 6 = 1: 76 => 1 case
.text
...
user_code_begin_000:
	...
	nop
LOOP_1: !! This loop is executed when  changing from 10 -> 20
	mov %g0, %i1 
  	sllx %l5, 1, %l5
	or %l5, %l6, %l2
	cmp %l5, 0x80
...
!! Also no err is flagged on a diagnostic read.
.text
...
!! check if cerer.scaU is clear or ceter.pscce is clear then the error is
...
!! Now set CERER.SCAU, but clear CETER.PSCCE. CHk that no error again.
  setx 0x20000000, %g4, %o2   !! enable scac errs
  add %g0, CERER_VA, %g3        !!g3 has cerer va
...
user_code_begin_005:
!!Now again inject errors in all the entries, but set cerer.scac bit.
!! Chk the behavior.
  add %g0, 0x5, %l2
  set SCAU_ERR_EN, %l3
  or %l3, %l2, %l3   
...
!! Now set CERER.SCAU, and see that the error is flagged.
!! Set cerer.scac bit
  setx 0x20000000, %g1, %o2   !! enable scac errs
  add %g0, CERER_VA, %g3        !!g3 has cerer va
  stxa %o2, [%g3]ASI_CERER
...
CHK_ERR_TYP:
  ...
  and %o1, 0xF, %o1
  cmp %o1, SCAU_ERR_TYPE
  bne FAIL
CHK_INDEX:
  add %g0, SFAR_VA, %g1
  ldxa [%g1]ASI_SFAR, %o1   !! read the DSFAR
  srl %i1, 3, %o3       !! expected value of index
  and %o1, 0x7, %o2
  cmp %o2, %o3
  bne FAIL
...
CHK_SYNDROME:
  ...
  and %o2, 0x80, %o3    !! look at the overall parity. Shd be 1 for correctable errors.
  brnz %o3, FAIL
  and %o2, 0x7F, %o3
  brz %o3, FAIL		!! syndrome shdn't be 0 for UE.
