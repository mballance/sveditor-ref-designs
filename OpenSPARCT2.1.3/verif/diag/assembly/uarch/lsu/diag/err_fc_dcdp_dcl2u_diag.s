...
* 
* OpenSPARC T2 Processor File: err_fc_dcdp_dcl2u_diag.s
...
main:		/* test begin */
 ta    T_CHANGE_HPRIV
  ...
  setx 0x0000008000000800, %l0, %o2   !! enable DCDP errs and l2c errors
...
user_code_begin_000:
  setx user_data_begin_000, %g1, %l1
  add %l1, 0x200, %l2 
  setx  TEST_DATA0, %g1, %g3
  setx  ALL_F,  %g1, %g4
...
store_to_l2_th0:
  ...
  !!add %l2, 0x10, %l2
  !!ldxa [%l2]0x10, %l5
  prefetch [%l2], 0x18   !!add a prefetch ice so that the above store is committed before the
			 !!diagnostic read executes
...
ld_in_l1_th0:
  ldxa [%l1]0x10, %l5  !! shd see data_acc_err, put line in cache with bad parity
  brz %i0, FAIL
  mov %g0, %i0
  ldxa [%l1]0x10, %l5  !! Do a ld, shd see both data_acc and hw_corrected err
  brz %i0, FAIL
  brz %i1, FAIL
  add %l2, 0x40, %l2
  dec %l4
  brnz %l4, store_to_l2_th0
  add %l1, 0x40, %l1
