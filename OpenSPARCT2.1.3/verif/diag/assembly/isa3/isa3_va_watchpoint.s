...
* 
* OpenSPARC T2 Processor File: isa3_va_watchpoint.s
...
main:		/* test begin */
	...
	ldx	[%l0], %l1	/* load first so that tlb doesn't miss next time */
	...
	/* Set data watchpoint */
st_asi:	wr	%g0, ASI_DMMU_VA_WATCHPOINT,	%asi
	stxa	%l0, [ASI_DMMU_VA_WATCHPOINT_VAL] %asi

	/* Read data watchpoint */
	ldxa	[ASI_DMMU_VA_WATCHPOINT_VAL] %asi, %i0

	/* Enable load VA watchpoint */
en_va:
	/* Mask=0x55 */
	set	0x55, %l1
	sll	%l1, 25, %l1
...
!!	ldxa	[%g0] ASI_LSU_CONTROL, %l2
	or	%l1, 0x1f, %l1		/* keep caches and spec bit on */
