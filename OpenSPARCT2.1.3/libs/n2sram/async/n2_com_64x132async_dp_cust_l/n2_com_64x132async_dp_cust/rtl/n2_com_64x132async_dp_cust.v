...
// 
// OpenSPARC T2 Processor File: n2_com_64x132async_dp_cust.v
...
module n2_com_64x132async_dp_cust	(
   // clocks, scan
   wr_clk,  
   rd_clk,
   wr_pce, 
...
input           scan_in;		//  
input           tcu_se_scancollar_in;	// scan enable to l1clk hdrs
input           tcu_scan_en;		// scan enable to and_clk hdrs
input           tcu_pce_ov;            	// scan signals
input           tcu_aclk;		// 
input           tcu_bclk;		// 
input           tcu_array_wr_inhibit;	// Array write/read inhibit during scan
input		bist_clk_mux_sel ;	// Clk Hdr Mux Select for MBIST
input		bist_clk ;		// BIST Clk, connect to pcl2clk
output          scan_out;		// 


input	  [5:0]	wr_addr;		//  wr port address in
input		wr_en;			//  wr port enable
input	[131:0] din;			//  data in

input	  [5:0]	rd_addr;		//  rd port address in
...
//  input flops
//------------------------------------------------------------------------
wire	  [5:0]	rd_addr_array ;
wire	  [5:0]	wr_addr_array ;
wire		rd_en_d1 ;
wire		wr_en_d1 ;
wire		rd_en_array ;
wire		wr_en_array ;
...
cl_mc1_sram_msff_mo_8x	ff_rd ( .si(din_so[66]), .so(rd_so), 
				.l1clk(rd_l1clk), .and_clk(rd_and_l1clk),  
				.siclk(siclk), .soclk(soclk), 
				.d(rd), .mq(rd_en_d1)	);

cl_mc1_sram_msff_mo_8x	ff_rd_addr_5 (  .si(rd_so), .so(rd_addr_so[5]), 
				.l1clk(rd_l1clk), .and_clk(rd_and_l1clk),  
				.siclk(siclk), .soclk(soclk), 
				.d(rd_addr[5]), .mq(rd_addr_array[5])	);
cl_mc1_sram_msff_mo_8x	ff_rd_addr_4 (  .si(rd_addr_so[5]), .so(rd_addr_so[4]), 
				.l1clk(rd_l1clk), .and_clk(rd_and_l1clk),  
				.siclk(siclk), .soclk(soclk), 
				.d(rd_addr[4]), .mq(rd_addr_array[4])	);
cl_mc1_sram_msff_mo_8x	ff_rd_addr_3 (  .si(rd_addr_so[4]), .so(rd_addr_so[3]), 
				.l1clk(rd_l1clk), .and_clk(rd_and_l1clk),  
				.siclk(siclk), .soclk(soclk), 
				.d(rd_addr[3]), .mq(rd_addr_array[3])	);
cl_mc1_sram_msff_mo_8x	ff_rd_addr_2 ( 	.si(rd_addr_so[3]), .so(rd_addr_so[2]), 
				.l1clk(rd_l1clk), .and_clk(rd_and_l1clk),  
				.siclk(siclk), .soclk(soclk), 
				.d(rd_addr[2]), .mq(rd_addr_array[2])	);
cl_mc1_sram_msff_mo_8x	ff_rd_addr_1 ( 	.si(rd_addr_so[2]), .so(rd_addr_so[1]), 
				.l1clk(rd_l1clk), .and_clk(rd_and_l1clk),  
				.siclk(siclk), .soclk(soclk), 
				.d(rd_addr[1]), .mq(rd_addr_array[1])	);
cl_mc1_sram_msff_mo_8x	ff_rd_addr_0 ( 	.si(rd_addr_so[1]), .so(rd_addr_so[0]), 
				.l1clk(rd_l1clk), .and_clk(rd_and_l1clk),  
				.siclk(siclk), .soclk(soclk), 
...
cl_a1_msff_4x	ff_wr_addr_5 ( .si(wr_en_so), .so(wr_addr_so[5]), .l1clk(wr_l1clk), .siclk(siclk), .soclk(soclk), 
...
n2_com_64x132async_dp_cust_array	ptl_hb_async_ram(
...
endmodule	//n2_com_64x132async_dp_cust
